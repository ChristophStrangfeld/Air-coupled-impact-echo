...
%% read data of the measurements to quantify the influence of the metal plate and the nozzel stand
%%%%% create required folder path for corresponding data
    data.directory = ['influence_metal_plate\'];
    ...
    for repetition_index =1:size(timesignal,2)
        ...
        %%%%% windowed spectrum. The time signal is defined in 10 sub-signals with an overlap of 50 %
        [spectrum, f] = pwelch(timesignal(:,repetition_index),sample_rate/10,sample_rate/20,[],sample_rate);
        spectra_metal_plate(:,repetition_index) = spectrum;
    end
    %%%%% averaged the windowed FFT over the five repeated measurements, this smoothens the signal even more
    %%%%% combine the spectra of all computed sleepers and all conducted excitation types in one matrix
    spectrum_metal_plate(:,file_index) = mean(spectra_metal_plate(:,:),2);
    end
...
power_intact = reshape(power_share_normalised_high_frequency_band(:,[1,2],:),1,[]);
power_defect = reshape(power_share_normalised_high_frequency_band(:,[3:6],:),1,[]);

frequency = reshape(maximum_frequencies,1,[]);
frequency_intact = reshape(maximum_frequencies(:,[1,2],:),1,[]);
frequency_defect = reshape(maximum_frequencies(:,[3:6],:),1,[]);

% plot(-1,-1,'.g','markersize',12)
% hold on
% plot(-1,-1,'.r','markersize',12)

plot(power_intact,frequency_intact/1000,'.g','markersize',12)
hold on
plot(power_defect,frequency_defect/1000,'.r','markersize',12)

legend('intact sleeper', 'defect sleeper','location','northwest','fontsize',plotstyle.fontsize,'FontName',plotstyle.fontname)
grid on
...
%% plot spectrum for impact hammer actuation with 8 mm impact hammer and no metal plate (#1,#4), with metal plate only (#2,#5) and nozzel stand only (#3,#6)
figure(60)
plotstyle.fontsize = 10;
plotstyle.fontname = 'times new roman';
clf
...
measurement_index = 1;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'-r','linewidth',0.5)
hold on
measurement_index = 4;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'--r','linewidth',0.5)
measurement_index = 2;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'-b','linewidth',0.5)
measurement_index = 5;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'--b','linewidth',0.5)
measurement_index = 3;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'-g','linewidth',0.5)
measurement_index = 6;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'--g','linewidth',0.5)
grid on
...
ylabel('power density spectrum in dB')
set(gca,'fontsize',plotstyle.fontsize,'fontname',plotstyle.fontname)

legend('no plate','no plate','plate','plate','stand','stand','fontsize',plotstyle.fontsize,'fontname',plotstyle.fontname)


%% plot spectrum for 2.5 mm air nozzel actuation at 2 bar and accelerometer at 5 cm distance and no metal plate (#7) and with metal plate between (#8)
figure(61)
plotstyle.fontsize = 10;
plotstyle.fontname = 'times new roman';
clf
...
hold on
measurement_index = 8;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'-k','linewidth',0.5)
grid on
...
legend('no plate','plate','fontsize',plotstyle.fontsize,'fontname',plotstyle.fontname)


%% plot spectrum for 8 m  impact hammer and air nozzel. Accelerometer is always at X =10cm, actuation at 250 cm. There is no metal plate (#9,#12), metal plate directly at sensor (#10,#13), and metal plate direcly at actuation (#11,#14)
figure(62)
plotstyle.fontsize = 10;
plotstyle.fontname = 'times new roman';
clf
...
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'-c','linewidth',0.5)
measurement_index = 14;
plot(f/1000,pow2db(spectrum_metal_plate(:,measurement_index)),'-m','linewidth',0.5)
grid on
...
h = legend('hammer, no plate','hammer, plate at sensor','hammer, plate at actuator','air, no plate','air, plate at sensor','air, plate at actuator','fontsize',plotstyle.fontsize,'fontname',plotstyle.fontname)
set(h,'position',[ 0.74 0.83 .01 .01]);
set(gca,'fontsize',plotstyle.fontsize,'fontname',plotstyle.fontname)
